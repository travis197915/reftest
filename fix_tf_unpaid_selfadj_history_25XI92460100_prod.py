#!/usr/bin/env python3
r"""Item #63 follow-up (claim 25XI92460100) — extend the Item #38
"self-adjustment mistaken for history" correction to the claim it originally
MISSED. NO LLM. Idempotent. Scoped: run with ``--claim 25XI92460100``.

Why Item #38 missed 25XI92460100
--------------------------------
Item #38 (``fix_timely_filing_selfadjustment_history_prod.py``) only flips a
claim history→no-history when its ONLY same-DOS/same-provider match is a
same-base self-adjustment. For 25XI92460100 the duplicate-claim tool returned,
besides the claim's own self-adjustment (25XI92460101, same base 25XI924601):
  * 25XH14013000 — a DIFFERENT provider (Keortge Cook 000151972003 vs the
    audited claim's Jamison 000624522007) and different DOS range → never history;
  * 25XJ41666800 — same provider + same DOS but UNPAID (CLCL_CUR_STS 99 "Closed",
    no CLCL_PAID_DT).
Item #38's matcher did not require a *paid* prior claim, so it counted the unpaid
25XJ41666800 as genuine history — the claim was therefore NOT recognized as
self-adjustment-only and was left untouched, still calculating timely filing off
the adjusted current claim.

The fix here
------------
The history matcher additionally requires the prior claim to have been PAID
(CLCL_PAID_DT non-null), per RULE-010 ("the history claim was previously paid").
With that, 25XI92460100 has NO genuine claim in history (self-adjustment,
different-provider, and unpaid claims all excluded), so — exactly like the
Item #38 25XJ83640500 scenario — the correct no-history path is applied:
timely filing is measured off the ORIGINAL claim in audit (DOS 06/20/2025 →
received 10/24/2025) and the timely-filing denial is confirmed correct → CLEAN.

This is a SEPARATE, surgical follow-up. It does NOT modify the Item #38 script or
any other timely-filing fix; run this AFTER them.

What the auditor flagged (25XJ83640500, Wendy 7/23)
---------------------------------------------------
    "There is no claim in history. Timely filing should be calculated off the
     current claim Date of Service and Receive Date 07/31/2025 to 11/24/2025."
     "Agent did determine the claim was denied for timely filing correctly and
     should be clean."

The history matcher (``facets_get_duplicate_claim`` → ``_dup_hist``) counted the
claim's OWN adjustment as a "prior claim in history". A Facets adjustment shares
the same base claim number and differs only in the trailing adjustment sequence
(e.g. ``25XJ83640500`` vs ``25XJ83640501``); it is the SAME claim re-adjudicated,
not a separate prior submission. Because of that false match the calculator ran
off the adjustment's paid date (Steps 10 → 11 → 12) and reported "within the
limit", hiding the fact that — with no genuine history — timely filing is
measured off the CURRENT claim's DOS and the claim was correctly denied.

The fix
-------
``_dup_hist`` now excludes any candidate whose base claim number (CLCL_ID minus
its 2-digit adjustment sequence) equals the audited claim's base — a
self-adjustment is never its own history. For the claims that flip from
"has history" to "no history" the correct no-history path is (re)applied:

    Step 5  -> new-day beyond the limit (measured off the current DOS) — Met
    Step 10 -> "New Day claim denying for timely filing … allow the system to
               deny, skip to Step 15" — Error badge, non-defect
    Step 11 / 12 -> Not Applicable (calculator not reached)
    Verdict -> CLEAN (a correctly-applied timely-filing denial is not a defect)

Any wrong "within-limit but TF-denied" Error previously written on Step 5 (by
``fix_timely_filing_within_but_tf_denied_prod.py``) is overwritten by the correct
new-day determination here.

Only claims whose ONLY history match is a same-base self-adjustment (i.e. they
flip history → no-history under the corrected matcher) are touched. Everything
else is left exactly as-is. Run ``--dry-run`` (default) to list them;
``--apply`` writes.
...
def main() -> None:
    ...
    _p("── TF self-adjustment-history correction (UAT 25XJ83640500) [no LLM] ──")
    ...
    def _dup_hist(run) -> dict:
        ...
        cur_base = cur[:-2]   # base claim number (strip the 2-digit adjustment seq)

        def _dos_provider_match(fc) -> bool:
            """SAME date(s) of service AND same provider AND a PAID prior claim.

            RULE-010 only treats a history claim as timely-filing history when it
            was *previously paid* (CLCL_PAID_DT non-null). An unpaid / closed
            claim (e.g. CLCL_CUR_STS 99 "Closed" with no paid date) is not a
            prior timely submission, so it must NOT count as history — this is the
            gap that let claim 25XI92460100 slip past the original Item #38 fix
            (its only non-self-adjustment same-DOS/provider match was an UNPAID
            closed claim, so the claim was not recognized as self-adjustment-only).
            """
            if str(fc.get("CLCL_ID") or "") == cur:
                return False   # the claim itself is never its own duplicate
            if not _parse_date(fc.get("CLCL_PAID_DT")):
                return False   # unpaid / no paid date → not a paid prior claim
        ...
        def _is_self_adjustment(fc) -> bool:
            """A Facets adjustment of the SAME claim: same base claim number, only
            the trailing 2-digit adjustment sequence differs (e.g. …00 vs …01).
            It is the same claim re-adjudicated, never a prior claim in history."""
            fid = str(fc.get("CLCL_ID") or "")
            return bool(cur_base) and fid[:-2] == cur_base and fid != cur

        def _is_match(fc) -> bool:
            return _dos_provider_match(fc) and not _is_self_adjustment(fc)

        matches = [fc for fc in all_fc if _is_match(fc)]
        # Did the OLD (pre-fix) matcher have history only via a self-adjustment?
        # Those claims flip history → no-history and are the ones this script fixes.
        old_matches = [fc for fc in all_fc if _dos_provider_match(fc)]
        selfadj_flip = bool(old_matches) and not matches \
            and any(_is_self_adjustment(fc) for fc in old_matches)
    ...
    def _seed_one(run: RuleExecutionRun) -> tuple[bool, str]:
        ...
        # Affected iff the claim's ONLY history match was a same-base self-adjustment
        # (it flips history → no-history under the corrected matcher). This is the
        # 25XJ83640500 scenario; every other claim is left exactly as-is.
        selfadj_flip = C["dup"].get("selfadj_flip", False)
        why = ["selfadj-history"] if selfadj_flip else []
        if affected_only and not why:
            return False, "not affected — history is not a same-base self-adjustment"
    ...
    _p("\n────────────────────────────────────────────────────────────")
    _p(f"affected claims (same-base self-adjustment mistaken as history) = {affected_n}"
       f"  of {total} in-scope")
