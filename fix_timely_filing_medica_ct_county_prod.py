#!/usr/bin/env python3
r"""Apply the auditor UAT "Medica false-match + CT county" correction
(Wendy 7/23, claim 25XK20940100) to ONLY the affected Timely-Filing claims.
NO LLM. Idempotent.

This is a SEPARATE, surgical follow-up to the earlier Timely-Filing fix scripts
(those are left untouched). Two independent tool-level bugs the auditor flagged
on 25XK20940100 (CCI MEDICARE CHOICE 3 HMO, Danbury CT, DOS 12/03→recd 12/08/25):

  1. Step 4 false "Medica" group match (Bug: substring matching).  The
     ``check_timely_filing_deadline`` tool matches the group token "MEDICA" as a
     SUBSTRING, so any plan whose name contains "MEDICARE" or "MEDICAID" (e.g.
     "CCI MEDICARE …", "MGB MEDICAID …", "NHP RI MEDICAID …") is wrongly matched
     to the Medica health-plan group and given Medica's 180-day guideline.
     ConnectiCare/Medicare/Medicaid plans are NOT the Medica health plan. For
     these claims Step 4 must show "matches none of the listed special groups —
     answer 'No' and proceed to Step 5", every group row (incl. Medica) Not
     Applicable, and the standard INN/OON limit applies.

     Downstream effect the auditor saw (Step 10 Rule #32 "incorrect defect"):
     because Step 4 wrongly applied a group rule, the claim was pushed onto the
     group/history path into Step 10. Once the false Medica match is removed a
     within-limit new-day claim routes at Step 5 to Step 15 and Steps 10/11/12
     become Not Applicable — the bogus Step-10 timely-filing "defect" disappears.
     (Genuinely beyond-limit plans still flow to Step 10 with the standard limit.)

  2. Step 2/3 CT "planning region" county (Bug: geocoder returns a region).
     Connecticut replaced its 8 counties with 9 "planning regions", so the
     geocoder returns e.g. "Western Connecticut Planning Region" and the county
     lookup fails ("Geocoded county '…' not found in Counties"). Danbury CT is in
     FAIRFIELD county. For CT planning-region claims the member's real county is
     resolved from ZIP/city and surfaced in the Step 2 reasoning; the erroneous
     "planning region not found in Counties" text is removed from the Step 3
     bulletin detail. (Every affected CT claim's bulletin is date-out-of-range /
     "Not Met", so this correction never changes a bulletin outcome.)

Only claims that (a) carry a false-substring Medica match, or (b) carry a CT
planning-region geocode are touched. Everything else is left exactly as-is. Run
...
# ── Bug 1: false "Medica" substring match ──────────────────────────────────
# The check_timely_filing_deadline tool matches the group token "MEDICA" as a
# substring, so "MEDICARE"/"MEDICAID" plans wrongly match the Medica health plan.
# A plan is the genuine Medica group ONLY when "MEDICA" appears as a whole word.
_MEDICA_WORD = re.compile(r"(?<![A-Z])MEDICA(?![A-Z])")


def _is_medica_false(dl: dict) -> bool:
    """True when the tool matched the Medica group but the plan only contains the
    'MEDICA' token via a MEDICARE/MEDICAID substring (not the Medica health plan)."""
    if not isinstance(dl, dict):
        return False
    if str(dl.get("matched_group") or "").upper() != "MEDICA":
        return False
    return _MEDICA_WORD.search(str(dl.get("plds_desc") or "").upper()) is None


def _medica_note(plds: str) -> str:
    up = (plds or "").upper()
    kind = "MEDICARE" if "MEDICARE" in up else ("MEDICAID" if "MEDICAID" in up else "MEDICARE/MEDICAID")
    return (f"the plan name contains '{kind}' but is not the Medica health plan, "
            f"so no special group applies")


# ── Bug 2: CT "planning region" → real county ──────────────────────────────
# CT replaced its 8 counties with 9 "planning regions"; the geocoder returns a
# planning-region name that is not in the bulletin county list. Resolve the
# member's real county from ZIP (authoritative), then city.
_CT_ZIP_COUNTY = {
    "06484": "Fairfield",       # Shelton
    "06088": "Hartford",        # East Windsor
    "06475": "Middlesex",       # Old Saybrook
    "06615": "Fairfield",       # Stratford
    "06611": "Fairfield",       # Trumbull
    "06238": "Tolland",         # Coventry
    "06790": "Litchfield",      # Torrington
    "06051": "Hartford",        # New Britain
    "06232": "Tolland",         # Andover
    "06460": "New Haven",       # Milford
    "06905": "Fairfield",       # Stamford
    "06776": "Litchfield",      # New Milford
    "06605": "Fairfield",       # Bridgeport
    "06831": "Fairfield",       # Greenwich
    "06811": "Fairfield",       # Danbury
}
_CT_CITY_COUNTY = {
    "SHELTON": "Fairfield", "EAST WINDSOR": "Hartford", "OLD SAYBROOK": "Middlesex",
    "STRATFORD": "Fairfield", "TRUMBULL": "Fairfield", "COVENTRY": "Tolland",
    "TORRINGTON": "Litchfield", "NEW BRITAIN": "Hartford", "ANDOVER": "Tolland",
    "MILFORD": "New Haven", "STAMFORD": "Fairfield", "NEW MILFORD": "Litchfield",
    "BRIDGEPORT": "Fairfield", "GREENWICH": "Fairfield", "DANBURY": "Fairfield",
}


def _ct_county(zipc: str, city: str) -> str:
    z = (zipc or "").strip()[:5]
    if z in _CT_ZIP_COUNTY:
        return _CT_ZIP_COUNTY[z]
    return _CT_CITY_COUNTY.get((city or "").strip().upper(), "")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Apply the auditor UAT Medica-substring + CT-county correction "
        "(Wendy 7/23, claim 25XK20940100) to ONLY the affected Timely-Filing claims. "
        "No LLM."
    )
    ...
    ap.add_argument("--all", action="store_true",
                    help="process every in-scope claim, not just the affected ones "
                         "(default: only claims with a false Medica substring match or a "
                         "CT planning-region county).")
    ...
    _p("── TF Medica-substring + CT-county correction (UAT 25XK20940100) [no LLM] ──")
    ...
    def _eval(step: int, bk: str, C: dict) -> tuple[str, str]:
        ...
        if step == 2:
            if st["found"]:
                ...
                # For CT the geocoder returns a "planning region" that is not a
                # county; surface the member's real county (resolved from ZIP/city).
                place = (f"{st['city'].title()} ({cty} County) {st['zip']}"
                         if cty else f"{st['city'].title()} {st['zip']}")
                return "MET", (
                    f"Member resides in {st['full']} ({st['abbr']}), "
                    f"{place} (check_member_address_and_state, "
    ...
    def _seed_one(run: RuleExecutionRun) -> tuple[bool, str]:
        ...
        # ── Bug 1: neutralize the false "Medica" substring match ──────────
        # A MEDICARE/MEDICAID plan is not the Medica health plan, so clear the
        # tool's group match; Step 4 then answers 'No' → proceed to Step 5 and
        # the standard INN/OON limit governs (the flow no longer detours to the
        # Step-10 group/history path, removing the bogus Step-10 "defect").
        medica_fix = _is_medica_false(C["deadline"])
        if medica_fix:
            dlc = dict(C["deadline"])
            dlc["matched_group"] = ""
            dlc["matched_subrule"] = ""
            dlc["group_match_detail"] = _medica_note(facts["plds"])
            C["deadline"] = dlc

        # ── Bug 2: resolve the CT "planning region" to the real county ────
        _sd = _tool(run, "check_timely_filing_state_date") or {}
        _region = ""
        for _lr in (_sd.get("line_results") or []):
            if "planning region" in str(_lr.get("geocoded_city") or "").lower():
                _region = str(_lr.get("geocoded_city")); break
        ct_county = ""
        if _region and (C["state"].get("abbr") or "").upper() == "CT":
            ct_county = _ct_county(C["state"].get("zip", ""), C["state"].get("city", ""))
        ct_fix = bool(ct_county)
        if ct_fix:
            C["state"]["county"] = ct_county
            # Drop the erroneous "planning region … not found in Counties" text
            # from the Step-3 bulletin detail (only when the bulletin does not
            # apply, which is the case for every affected CT claim).
            if not C["bulletin"]["met"]:
                C["bulletin"]["detail"] = (
                    f"member resides in {ct_county} County "
                    f"({C['state'].get('city', '').title()}, CT); the claim's date(s) of "
                    f"service are not within any active Emergency Response Bulletin "
                    f"effective range")

        C["reached"], C["path"] = _reached(C)

        # ---- affected-claim gate (Wendy 7/23, claim 25XK20940100) -----------
        # (a) the plan carries a false-substring Medica group match, or
        # (b) the member's CT geocode returned a "planning region" county.
        why = [("medica-group" if medica_fix else None),
               ("ct-county" if ct_fix else None)]
        why = [w for w in why if w]
        if affected_only and not why:
            return False, ("not affected — no false Medica substring match and no CT "
    ...
    _p("\n────────────────────────────────────────────────────────────")
    _p(f"affected claims (false Medica match or CT planning-region)= {affected_n}"
       f"  of {total} in-scope")
