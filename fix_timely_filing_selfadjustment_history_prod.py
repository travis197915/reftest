#!/usr/bin/env python3
r"""Apply the auditor UAT "self-adjustment mistaken for history" correction
(claim 25XJ83640500, tracker Step-10 defect) to ONLY the affected Timely-Filing
claims. NO LLM. Idempotent.

This is a SEPARATE, surgical follow-up. It does NOT modify
``fix_timely_filing_steps_gate_prod.py``, ``fix_timely_filing_deny_disclaimer_prod.py``
or ``fix_timely_filing_within_but_tf_denied_prod.py`` (all left untouched); run
this AFTER them.
...
def main() -> None:
    ...
    def _dup_hist(run) -> dict:
        ...
        def _dos_provider_match(fc) -> bool:
            """SAME date(s) of service AND same provider (the old criterion)."""
